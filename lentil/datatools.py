...
class Interaction(object):
    ...
    def __init__(
        ...
        should always be initialized with non-None values for user_id,
        module_id, timestep, and module_type

        :param str user_id: A user_id
...
class SplitHistory(object):
    ...
    def __init__(
        ...
        :param (np.ndarray,np.ndarray,np.ndarray) assessment_interactions:
            A tuple of (user_idxes, module_idxes, outcomes)

        :param (np.ndarray,np.ndarray,np.ndarray) lesson_interactions:
            A tuple of (user_idxes, module_idxes, times_since_previous_interaction)

        :param dict[str,int] timestep_of_last_interaction:
            A dictionary mapping user_id to the timestep of the student's last interaction
        """
...
class InteractionHistory(object):
    ...
    def __init__(
        ...
        if sort_by_timestep:
            data.sort_values(by='timestep', axis=0, inplace=True)
        ...
        all_user_ids = self.data['user_id'].unique()
        random.shuffle(all_user_ids)
        num_nontest_students = int((1 - size_of_test_set) * len(all_user_ids))
        self.id_of_nontest_user_idx = {i: k for i, k in enumerate(
            all_user_ids[:num_nontest_students])}
    ...
    def reindex_timesteps(self):
        ...
        new_timesteps_for_student = {user_id: {k: (i+1) for i, k in enumerate(
            sorted(group['timestep'].unique()))} for user_id, group in self.data.groupby(
                by='user_id')}
        self.data['timestep'] = self.data.apply(
                lambda ixn: new_timesteps_for_student[ixn['user_id']][ixn['timestep']],
                axis=1)
    ...
    def compute_idx_maps(
        ...
        :param dict[str,int]|None user_idx: A dictionary mapping user_id to user_idx
        ...
        self._user_idx = buildidx(user_ids) if user_idx is None else user_idx
        ...
        self._student_inv_idx = build_inv_idx(self._user_idx)
    ...
    def squash_timesteps(self, num_checkpoints=10):
        ...
        df = self.data.groupby('user_id')

        for user_idx, user_id in enumerate(self.iter_students()):
            if checkpoint_len > 0 and user_idx % checkpoint_len == 0:
                _logger.debug("Processed student %d of %d", user_idx, num_students)
            timestep = 1
            for idx, interaction in df.get_group(user_id).iterrows():
    ...
    def split_interactions_by_type(
        ...
        def df_to_unique_timesteps(dataframe):
            ...
            output = dataframe['user_id'].apply(self.idx_of_user_id).values
        ...
        if len(lesson_df) == 0 and insert_dummy_lesson_ixns:
            ...
            for user_id, group in df.groupby('user_id'):
                ...
                user_idxes_of_lesson_ixns.extend([self.idx_of_user_id(user_id)] * (T-1))
            ...
            user_idxes_of_lesson_ixns = np.array(user_idxes_of_lesson_ixns)
            times_since_prev_ixn_of_lesson_ixns = np.array(times_since_prev_ixn_of_lesson_ixns)
        else:
            user_idxes_of_lesson_ixns = np.array(lesson_df['user_id'].apply(
                self.idx_of_user_id)) * num_timesteps + np.array(lesson_df['timestep'])
        ...
        lesson_interactions = (user_idxes_of_lesson_ixns,
                lesson_idxes_of_lesson_ixns,
                times_since_prev_ixn_of_lesson_ixns)

        timestep_of_last_interaction = df.groupby('user_id')['timestep'].max().to_dict()
        num_students = self.num_students()
    ...
    def id_of_user_idx(self, user_idx):
        """
        Get student id of a student index

        :param int user_idx: An index of a student
        :rtype: str
        :return: The id of the student at that idx
        """
        return self._student_inv_idx[user_idx]
    ...
    def idx_of_user_id(self, user_id):
        """
        Get student index of a student id

        :param str user_id: A student id
        :rtype: int
        :return: The student's index
        """
        return self._user_idx[user_id]
    ...
    def iter_students(self):
        ...
        """
        return self._user_idx.iterkeys()
    ...
    def module_sequence_of_student(self, user_id):
        """
        ...
        """
        return self.data['module_id'][self.data['user_id']==user_id]
